...
class buxiangqumingzi(nn.Module):
    def __init__(self, args):
        ...
        self.mlp = MultiLayerPerceptron(args.lsd_dim, args.classes)

    def forward(self, x, missing_index):
        output = self.encoder(x, missing_index)
...
if __name__ == '__main__':
    ...
    epochs = 200
    # Net
    net = buxiangqumingzi(args).to(args.device)
    # 优化器
    ...
    for epoch in range(epochs):
        ...
        for step, (idx, X, y, missing_index) in enumerate(train_loader):
            missing_index = torch.ones(missing_index.shape)
            y = y.to(args.device)
            output = net(X, missing_index)
        ...
        with torch.no_grad():
            for step, (idx, X, y, missing_index) in enumerate(eval_loader):
                missing_index = torch.ones(missing_index.shape)
                y = y.to(args.device)
                lsd = net.encoder(X, missing_index)
                lsds = torch.concat([lsds, lsd])
                output = net.mlp(lsd)
